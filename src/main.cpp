...
volatile long speed_counter =
        0; // A long integer which will store the value of the speed counter.
...
int main(int argc, char *argv[]) {
    ...
    LOCK_VARIABLE(speed_counter); // Used to set the timer - which regulates the
    // game's speed
    ...
    /*Background*/
    BITMAP *background = load_bitmap("res/images/Area1.bmp", NULL);

    /*Fighter Image*/
    BITMAP *gameSprite = load_bitmap("res/images/ken.bmp", NULL);
    /*Enemy Images*/
    BITMAP *EnemySprite[3];
    EnemySprite[0] = load_bitmap("res/images/enemy1.bmp", NULL);
    EnemySprite[1] = load_bitmap("res/images/enemy2.bmp", NULL);
    EnemySprite[2] = load_bitmap("res/images/enemy3.bmp", NULL);
    // Game Health Bar
    BITMAP *ui = load_bitmap("res/images/ui.bmp", NULL);

    if (gameSprite == NULL || background == NULL || ui == NULL
        || EnemySprite[0] == NULL
        || EnemySprite[1] == NULL
        || EnemySprite[2] == NULL
            ) {
    ...
    SAMPLE *gameLose = load_sample("res/sounds/lost.wav");
    if (!gameLose)
    ...
    while (!key[KEY_ESC]) {
        ...
        if (screenMode->GetState() == INITIALIZE) {
            /*
             * All the objects are instantiated within this screen mode. This is
             * crucial for Game restart without discontinuity
             */
        ...
        if (screenMode->GetState() == SPLASH) {
            ...
            while (speed_counter > 0) {
                ...
                if (key[KEY_RIGHT]) // If the user hits the right key, change the
                    // picture's X coordinate
                {
                ...
                } else if (key[KEY_S]) // If the user hits the up key, change the
                    // picture's Y coordinate
                {
